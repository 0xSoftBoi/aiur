...
- `p0a_funnel.stl` — Ø180 mm thin-wall funnel, Ø16 mm throat, Ø70 mm drill-after-print flange;
- `p0a_probe_head.stl` — Ø12 mm rounded coupon head with a Ø3.2 mm bore for a Ø3 mm mast;
- `p0a_keeper.stl` — 2.5 mm sliding fork with a 4.2 mm slot around the nominal Ø3 mm neck;
- `p0a_drill_template.svg` — 1:1 four-hole M3 template on a 40 mm square;
- `p0a_rev_a_manifest.json` — dimensions, mesh checks, volume, and a solid-PETG mass estimate.
...
4. Print `p0a_drill_template.svg` at exactly 100%. Measure the 50 mm check line with calipers before using it. Drill the four flange holes Ø3.2 mm only after the funnel is printed.
5. Install the keeper in rigid guides and translate it manually through at least 11 mm of travel. The guides and closed stop react load; the actuator must never be the structural retention path.
