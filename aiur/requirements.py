...
REQUIREMENTS: tuple[Requirement, ...] = (
    ...
    # ---- STRATO-P0: the observation article (docs/prototype-strato-p0.md) ----
    Requirement(
        "S0-ATM-001",
        "predicted burst altitude of the reference sounding article",
        ">= 25 km in the standard atmosphere, i.e. >= 5 km of margin over the "
        "20 km stratospheric threshold",
        "resize the balloon or cut package mass before buying gas",
        "executable ascent model in aiur/strato.py",
        VerificationMethod.ANALYSIS,
        Stage.REQUIREMENT,
        None,
        None,
        ClosureStatus.CLOSED,
        "aiur/strato.py SoundingBalloon.burst_altitude_m + tests/test_strato.py "
        "burst-margin case; model only — re-open against the first flight's "
        "measured burst altitude",
    ),
    Requirement(
        "S0-ENV-001",
        "coldest internal temperature reached during the bench cold soak",
        "<= -55 degC, the standard-atmosphere temperature at 20 km, held for >= 3 h",
        "the package does not fly; add insulation or heat and re-soak",
        "chamber log time-aligned with the package's internal temperature channel",
        VerificationMethod.TEST,
        Stage.BENCH_HIL,
        "S0-A",
    ...
    Requirement(
        "S0-ENV-002",
        "imaging, tracking, or termination dropouts during the cold soak",
        "0 dropouts",
        "the package does not fly; the dropout is a defect, not a flight risk to carry",
        "package health log across the full soak",
        VerificationMethod.TEST,
        Stage.BENCH_HIL,
        "S0-A",
    ...
    Requirement(
        "S0-FLT-001",
        "maximum altitude reached by the package on a free flight",
        ">= 20,000 m by GNSS, the lowest altitude that is stratospheric at every latitude",
        "the flight does not count toward S0-C; re-run the ascent model against the "
        "measured profile",
        "GNSS altitude channel of the flight log",
        VerificationMethod.TEST,
        Stage.FREE_FLIGHT,
        "S0-C",
        "max_altitude_m",
    ...
    Requirement(
        "S0-FLT-002",
        "recovery of the payload package after a free flight",
        "package recovered with its stored imagery intact",
        "the flight does not count toward S0-C; the loss is a hazard-log event",
        "recovery record with the article identity and storage checksum",
        VerificationMethod.TEST,
        Stage.FREE_FLIGHT,
        "S0-C",
        "payload_recovered",
    ...
    Requirement(
        "S0-FLT-003",
        "distance between the actual landing point and the pre-launch prediction",
        "<= 15 km",
        "no further free flight until the prediction workflow is corrected",
        "pre-launch prediction file committed before release, versus the recovery GNSS fix",
        VerificationMethod.TEST,
        Stage.FREE_FLIGHT,
        "S0-C",
        "landing_error_km",
    ...
    Requirement(
        "S0-FLT-004",
        "tethered ascents completed before the first free flight",
        ">= 3 tethered ascents of the flight configuration",
        "no free flight",
        "tethered run records with the article identity",
        VerificationMethod.TEST,
        Stage.TETHERED_FLIGHT,
        "S0-B",
        "tethered_flights",
    ...
    Requirement(
        "S0-LINK-001",
        "longest telemetry gap during a free flight",
        "<= 300 s",
        "the flight does not count toward S0-C; the link margin is re-budgeted",
        "ground-station receive log against the package's transmit log",
        VerificationMethod.TEST,
        Stage.FREE_FLIGHT,
        "S0-C",
        "telemetry_gap_max_s",
    ...
    Requirement(
        "S0-LINK-002",
        "longest position-report gap during a tethered ascent",
        "<= 60 s",
        "no free flight until the tracker is continuous on the tether",
        "ground-station receive log",
        VerificationMethod.TEST,
        Stage.TETHERED_FLIGHT,
        "S0-B",
    ...
    Requirement(
        "S0-MASS-001",
        "payload-package allocation against the program ceiling",
        "<= 1.0 kg allocation, with the baseline allocation <= 850 g; hard ceiling "
        "1.814 kg (4 lb) from the program's reading of 14 CFR 101.1(a)(4)",
        "cut allocation before adding hardware; never spend the ceiling",
        "executable mass budget in aiur/strato.py",
        VerificationMethod.ANALYSIS,
        Stage.REQUIREMENT,
        None,
        None,
        ClosureStatus.CLOSED,
        "aiur/strato.py baseline_strato_budget + tests/test_strato.py allocation "
        "case; allocation only — re-open when measured package masses exist",
    ),
    Requirement(
        "S0-MASS-002",
        "measured mass of the complete flight-ready payload package",
        "<= 1.814 kg (4 lb)",
        "the package does not fly",
        "scale measurement of the assembled package recorded with the article identity",
        VerificationMethod.TEST,
        Stage.BENCH_HIL,
        "S0-A",
    ...
    Requirement(
        "S0-OBS-001",
        "observation frames stored above the stratospheric threshold with a valid "
        "position tag",
        ">= 50 frames per free flight",
        "the flight does not count toward S0-C",
        "frame index cross-referenced with the GNSS log",
        VerificationMethod.TEST,
        Stage.FREE_FLIGHT,
        "S0-C",
    ...
    Requirement(
        "S0-OBS-002",
        "nadir ground sample distance of the reference optic at 20 km",
        "<= 3.0 m",
        "change the optic before it is bought",
        "executable observation geometry in aiur/strato.py",
        VerificationMethod.ANALYSIS,
        Stage.REQUIREMENT,
        None,
        None,
        ClosureStatus.CLOSED,
        "aiur/strato.py Optic.nadir_gsd_m + tests/test_strato.py GSD case; "
        "geometry only — resolution is measured on a ground target after S0-C",
    ),
    Requirement(
        "S0-OBS-003",
        "end-to-end image captures stored and thumbnailed during the cold soak",
        ">= 100 captures",
        "the package does not fly",
        "image index and thumbnail log from the soak",
        VerificationMethod.TEST,
        Stage.BENCH_HIL,
        "S0-A",
    ...
    Requirement(
        "S0-PWR-001",
        "battery energy carried against the predicted sounding-flight draw",
        ">= 1.5 x predicted flight energy after the cold-capacity derate",
        "add cells or cut load; never fly on nominal capacity",
        "executable battery budget in aiur/strato.py",
        VerificationMethod.ANALYSIS,
        Stage.REQUIREMENT,
        None,
        None,
        ClosureStatus.CLOSED,
        "aiur/strato.py sounding_battery_wh + tests/test_strato.py battery case; "
        "the cold-capacity fraction is a placeholder until S0-A measures it",
    ),
    Requirement(
        "S0-SAFE-001",
        "flight termination when commanded",
        "0 failures over >= 10 bench trials",
        "the package does not fly until termination is unconditional",
        "per-trial termination record",
        VerificationMethod.DEMONSTRATION,
        Stage.BENCH_HIL,
        "S0-A",
    ...
    Requirement(
        "S0-SAFE-002",
        "flight termination with the payload computer powered off",
        "termination fires from its independent timer or radio path with the main "
        "computer unpowered",
        "the package does not fly; a termination path that needs the computer is not one",
        "bench demonstration record with the computer's power channel logged off",
        VerificationMethod.DEMONSTRATION,
        Stage.BENCH_HIL,
        "S0-A",
    ...
    Requirement(
        "S0-SAFE-003",
        "descent rate of the package under its parachute at sea-level density",
        "<= 5.0 m/s",
        "resize the canopy; do not fly a package that lands faster",
        "instrumented drop test of the flight package and canopy",
        VerificationMethod.TEST,
        Stage.BENCH_HIL,
        "S0-A",
    ...
    Requirement(
        "S0-SAFE-004",
        "parachute deployment after an in-flight termination on the tether",
        "0 deployment failures",
        "no free flight",
        "tethered run record and video corroboration",
        VerificationMethod.DEMONSTRATION,
        Stage.TETHERED_FLIGHT,
        "S0-B",
        "parachute_failures",
    ...
    Requirement(
        "S0-SAFE-005",
        "airspace coordination for the launch",
        "notice or authorisation required by the launch jurisdiction on file before "
        "release, with the launch window and predicted trajectory attached",
        "no release",
        "pre-launch checklist record naming the authority, reference, and time",
        VerificationMethod.INSPECTION,
        Stage.FREE_FLIGHT,
        "S0-C",
    ...
    Requirement(
        "S0-SAFE-006",
        "contact between the balloon, line, or package and any third party",
        "0 contacts with a person, vehicle, or structure outside the crew",
        "stop the campaign; the event goes to the hazard log before any further flight",
        "recovery record, landowner contact log, and video where available",
        VerificationMethod.TEST,
        Stage.FREE_FLIGHT,
        "S0-C",
